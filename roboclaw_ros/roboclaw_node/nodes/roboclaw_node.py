...
from roboclaw_driver.roboclaw_driver import Roboclaw as roboclaw
import rospy
import tf
...
class EncoderOdom:
    ...
    def update_publish(self, enc_left, enc_right):
        ...
        if abs(enc_left - self.last_enc_left) > 20000:
            ...
            vel_x, vel_theta = self.update(enc_left, enc_right) #prueba
...
class Node:
    def __init__(self):
        ...
        self.direction = int(rospy.get_param("~direction","0")) #Sentido de giro de las ruedas
        ...
        self.TICKS_PER_METER = float(rospy.get_param("~tick_per_meter", "5000"))
...
#         rospy.loginfo("linear.x %d vl_ticks: %d", linearÇ, vl_ticks)

        #Añadimos el sentido de giro
...
#       
        ...
        #Añdimos condiciones para linear_y
...
#         rospy.loginfo("linear_y: %d ", linear_y)
#         rospy.loginfo("angular_z: %d ", angular_z)
        #Fin
#         
#         vr =  linear_x -linear_y + angular_z * self.BASE_WIDTH #/ 2.0  # m/s  AÑADIMOS LINEAR_Y
#         vl = linear_x +linear_y - angular_z * self.BASE_WIDTH #/ 2.0  #AÑADIMOS LINEAR_Y
#         
#         rospy.loginfo("vr: %f ", vr)
#         rospy.loginfo("vl: %f ", vl)
#         elif self.rob =="2":                #CONTROLADORA TRASERA 2x15
        vr = linear_x +linear_y + twist.angular.z * self.BASE_WIDTH #/ 2.0  # m/s  AÑADIMOS LINEAR_Y
        vl = linear_x -linear_y - twist.angular.z * self.BASE_WIDTH #/ 2.0  #AÑADIMOS LINEAR_Y
...
#         rospy.loginfo("TICKS_PER_METER: %d", self.TICKS_PER_METER)
        rospy.loginfo("ADDRESS:%s  vr_ticks:%d vl_ticks: %d", self.address, vr_ticks, vl_ticks)
        ...
        try:
            ...
            if vr_ticks == 0 and vl_ticks == 0:
                ...
                if not self.roboclaw.SpeedM1M2(self.address, vr_ticks, vl_ticks):
                    rospy.loginfo("FALLO")
